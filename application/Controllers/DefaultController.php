...
class DefaultController extends Controller {
	...
	public function index ($request) {
		...
		// set variables
		$this->engine->assign("total_members", $database->fetchOne("SELECT COUNT(memberID) FROM ".DB_PREFIX."members"));
		$this->engine->assign("total_subscribers", $database->fetchOne("SELECT COUNT(subscriberID) FROM ".DB_PREFIX."subscribers"));
		$this->engine->assign("total_events", $database->fetchOne("SELECT COUNT(eventID) FROM ".DB_PREFIX."events"));
		$this->engine->assign("total_pages", $database->fetchOne("SELECT COUNT(pageID) FROM ".DB_PREFIX."pages"));
		$this->engine->assign("total_blog_posts", $database->fetchOne("SELECT COUNT(postID) FROM ".DB_PREFIX."blog_posts"));
